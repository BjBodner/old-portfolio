function [f] = OneD_Triple_Current_Plot_Generator(Average_NumberOfCharges,Average_Current,Average_AppliedVoltage,NumberOfVoltageTests,ThresholdVoltage,Average_Calculated_Theshold_Voltage,FirstChargedSystem,InitialSystem,Time,Average_ChargesTransfered,  Current_Standard_Deviation)
...
Average_AppliedVoltage_forAverage_Current = Average_AppliedVoltage*min(Average_Current);
plot(ax2,Time, Average_Current,'-sb')
hold on
plot(ax2,Time,Average_AppliedVoltage_forAverage_Current,'-rs')
hold off
title(ax2,'Avergage Current in system as a function of time')
ylabel(ax2,'Average_Current');
xlabel(ax2,'Time (1/Energy)');
ylim(ax2,[0 1.05*max(Average_Current)])

ax3 = subplot(3,1,3);

Average_AppliedVoltage_forAverage_Current = Average_AppliedVoltage*min(Current_Standard_Deviation);
plot(ax3,Time, Current_Standard_Deviation,'-s',Time,Average_AppliedVoltage_forAverage_Current,'-rs')
title(ax3,'Starndard deviation ofcurrent')
ylabel(ax3,'Current Standard Deviation')
xlabel(ax3,'Time ');
