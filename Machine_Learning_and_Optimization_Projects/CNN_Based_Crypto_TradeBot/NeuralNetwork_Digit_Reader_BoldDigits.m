function   [ChosenDigit,Confidence] = NeuralNetwork_Digit_Reader_BoldDigits(Digit)
...
for k = 1:78
    if k <=8
        [Y,~,~] = NN_BoldLetters_10Neurons(Expanded_X(:,k),1,1);
    end
    if k > 8
        if k <=16
            [Y,~,~] = NN_BoldLetters_12Neurons(Expanded_X(:,k-8),1,1);
        end
    end
    
    if k > 16
        if k <= 24
            [Y,~,~] = NN_BoldLetters_14Neurons(Expanded_X(:,k-16),1,1);
        end
    end
    if k > 24
        if k <= 32
            [Y,~,~] = NN_BoldLetters_16Neurons(Expanded_X(:,k-24),1,1);
        end
    end
    if k > 32
        if k <= 40
            [Y,~,~] = NN_BoldLetters_18Neurons(Expanded_X(:,k-32),1,1);
        end
    end
    if k > 40
        if k <= 48
            [Y,~,~] = NN_BoldLetters_20Neurons(Expanded_X(:,k-40),1,1);
        end
    end
    ...
    if q <=5
        [Y,~,~] = NN_BoldLetters_22Neurons(Expanded_X(:,q),1,1);
    end
    if q > 5
        if q <=10
            [Y,~,~] = NN_BoldLetters_24Neurons(Expanded_X(:,q-5),1,1);
        end
    end
    
    if q > 10
        if q <= 15
            [Y,~,~] = NN_BoldLetters_26Neurons(Expanded_X(:,q-10),1,1);
        end
    end
    if q > 15
        if q <= 20
            [Y,~,~] = NN_BoldLetters_28Neurons(Expanded_X(:,q-15),1,1);
        end
    end
    if q > 20
        if q <= 25
            [Y,~,~] = NN_BoldLetters_30Neurons(Expanded_X(:,q-20),1,1);
        end
    end
    if q > 25
        if q <= 30
            [Y,~,~] = NN_BoldLetters_10Neurons(Expanded_X(:,q-25),1,1);
        end
    end
