function   [ChosenDigit] = NeuralNetwork_Digit_Reader2(BitcoinDigit)
...
for k = 1:25
    if k <=5
        [Y,Xf,Af] = Limited_DataBase_NN_35Neurons(Expanded_X(:,k),1,1);
    end
    if k > 5
        if k <=10
            [Y,Xf,Af] = Limited_DataBase_NN_30Neurons(Expanded_X(:,k-5),1,1);
        end
    end
    
    if k > 10
        if k <= 15
            [Y,Xf,Af] = Limited_DataBase_NN_25Neurons(Expanded_X(:,k-10),1,1);
        end
    end
    if k > 15
        if k <= 20
            [Y,Xf,Af] = Limited_DataBase_NN_20Neurons(Expanded_X(:,k-15),1,1);
        end
    end
    if k > 20
        if k <= 25
            [Y,Xf,Af] = Limited_DataBase_NN_15Neurons(Expanded_X(:,k-20),1,1);
        end
    end
    Y_Vector(:,k) = Y;
