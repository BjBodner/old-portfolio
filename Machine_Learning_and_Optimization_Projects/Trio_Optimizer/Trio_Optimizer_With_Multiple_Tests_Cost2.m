...
for itteration = 1:Number_Of_Ittertions
    ...
    [CostVector0] = CostFunction2(ParametersArray);
    ParametersArray1 = ParametersArray;
    ...
    if RecomendedResourceAllocation(1) > 0 % Part 2 of Guessing_Game_Optimizer
        ...
        if UseCostFunction1 == 1
            [Cost_From_GG] = CostFunction(Suggested_Parameter_Samples_From_Guessing_Game);
        end
        if UseCostFunction2 == 1
            [Cost_From_GG] = CostFunction2(Suggested_Parameter_Samples_From_Guessing_Game);
        end
   ...
   % [CostVector1] = CostFunction2(ParametersArray);
   % b = sum(CostVector1 == CostVector0)/length(CostVector)
   % a = sum(sum(ParametersArray1 == ParametersArray))/(length(ParametersArray1(1,:))*length(ParametersArray1(:,1)))
    
    %% Finding Absolute Best Sample, and reseting the initial parameter vector
    [InitialCost1,I] = min(CostVector)
    if InitialCost1 < Cost_From_GG
    ...
    if InitialCost1 >= Cost_From_GG
        Cost_Change = InitialCost1 - InitialCost;
        Parameter_ChangeVector = Suggested_Parameter_Samples_From_Guessing_Game - Initial_Parameters ;
        Initial_Parameters = Suggested_Parameter_Samples_From_Guessing_Game;
    end
    InitialCost = InitialCost1;
    [InitialCost2] = CostFunction2(ParametersArray(:,I));
    TookTrueMinimum = (min(InitialCost1,Cost_From_GG) == InitialCost2)
    
    CostTracking(itteration) = InitialCost1;
    
sum(sum(ParametersArray1(:,I) == ParametersArray(:,I)))
...
CostTracking_OfAlgorithms(:,5) = CostTracking.';
CostTracking_OfAlgorithms1 = [ones(5,1)*InitialCost CostTracking_OfAlgorithms.'].';
...
figure
plot(n,CostTracking_OfAlgorithms(:,1),'-b',n,CostTracking_OfAlgorithms(:,2),'-r',n,CostTracking_OfAlgorithms(:,3),'-g',n,CostTracking_OfAlgorithms(:,4),'-c',n,CostTracking_OfAlgorithms(:,5),'-k')
legend('Guessing Game','Pinball','Two Mode','RandomSearch','Trio')
