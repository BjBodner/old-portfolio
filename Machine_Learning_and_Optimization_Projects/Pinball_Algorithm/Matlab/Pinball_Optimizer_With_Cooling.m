function [ParameterVector,CurrentCost,TotalTime] = Pinball_Optimizer_With_Cooling(ParameterVector,Search_Ratio,NumberOfItterations,NumberOfSamples__In_Each_Itteration,AmplitudeOfLinearSearch,AmplitudeOfRandomSearch,Readout)

% Defaults hyperparameters
NumberOfSamples__In_Each_Itteration = 20;
...
NumberOfCoolingSessions = 5;
cooling = 1;
for itteration = 1:NumberOfSamples__In_Each_Itteration
    
    if mod(NumberOfSamples__In_Each_Itteration,NumberOfCoolingSessions) ==0
        if cooling == 1
            Amplitude = 1;
        end
        if cooling > 1
        ...
        Maxchange = 0;
        Search_Ratio = 0.7-0.2*(cooling/10);
    ...
    Maxchange1 = max(abs(New_ParameterVector - Old_ParameterVector));
    if Maxchange1 > Maxchange
